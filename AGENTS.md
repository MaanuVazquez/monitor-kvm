...
- **Source layout**: `src/modules/webos/` contains the WebOS TV/Monitor library. `src/index.ts` re-exports the public API. `scripts/` contains CLI utilities.
- **Scripts**:
...
- **Public API** (`src/modules/webos/index.ts`): `pair()`, `connect()`, `forgetCredentials()`, plus the `WebOSClient` handle returned by `connect()`.
- **Dependencies**: `ws` (WebSocket client) and `@types/ws`. No other runtime deps.
